    ...
    <main className="relative bg-[var(--lv-dark)]">
      ...
      <section className="relative flex min-h-[100svh] items-end">
        ...
        <div
          ...
          aria-hidden
        />
        {/* Cinematic overlay — full black at bottom, tapers toward top */}
        <div
        ...
        <div className="relative z-10 mx-auto w-full max-w-6xl px-6 pb-16 sm:px-10 sm:pb-24 lg:px-16 lg:pb-32">
          ...
          {/* Main headline */}
          <h1 className="mt-5 max-w-[13ch] text-[2.55rem] font-light leading-[1.07] tracking-[-0.01em] text-[var(--lv-cream)] sm:text-5xl lg:max-w-2xl lg:text-6xl">
            {hotelConfig.heroTitle}
          </h1>

          {/* Subtitle */}
          <p className="mt-5 max-w-[28ch] text-sm leading-[1.7] text-[var(--lv-cream)]/52 sm:text-base">
          ...
          <div className="mt-9 flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
            ...
            <Link
              href="/booking"
              className="inline-flex items-center justify-center rounded-full bg-[var(--cabin-terra)] px-8 py-[14px] text-sm font-semibold tracking-wide text-[var(--lv-cream)] shadow-[0_10px_36px_rgba(176,68,48,0.42)] transition-all duration-200 ease-out hover:bg-[var(--cabin-terra-hover)] hover:shadow-[0_16px_52px_rgba(176,68,48,0.54)] sm:w-auto"
            >
              Reservar estancia
            </Link>

            {/* Secondary — ghost WhatsApp */}
            {process.env.NEXT_PUBLIC_WHATSAPP_BOOKING_URL && (
              <a
                href={process.env.NEXT_PUBLIC_WHATSAPP_BOOKING_URL}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center gap-2 rounded-full border border-white/16 bg-white/5 px-8 py-[14px] text-sm font-medium tracking-wide text-[var(--lv-cream)]/65 backdrop-blur-sm transition-all duration-200 ease-out hover:border-white/28 hover:text-[var(--lv-cream)]/85 sm:w-auto"
              >
                <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" className="shrink-0 opacity-75">
